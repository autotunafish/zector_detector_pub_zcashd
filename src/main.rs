...
//or input a new address.
    if pb_ip.len() != 0 {
        println!("\x1b[48;5;232m\x1b[38;5;157m* Or Hit Enter To Use Previous Input Recorded *\x1b[0m");
...
let getblockchaininfo = "\"getblockchaininfo\",\"params\": []}' | json_pp > ~/zector_detector_pub_zcashd/chaininfo2.txt";
	...
	//let getinfo = "\"getinfo\",\"params\": []}' | json_pp > ~/zector_detector_pub_zcashd/chaininfo2.txt";
...
//////////////////////////////////////
	...
	//This creates an inotify instance add_watch on the filename that receives the curl response.
			...
			inotify.add_watch(&ch2, WatchMask::CLOSE_WRITE).expect("Failed to add file watch");
	...
	//Most will print the matched item and the following two items as given by .next(), .next().
                            match kvc {
                                ...
                                "\"chainwork\"" | "\"blocks\"" | "\"size_on_disk\"" | "\"bestblockhash\"" => {
									println!(
                               ...
                               println!(
                                    "\x1b[48;5;232m\x1b[38;5;159m\"pool\":{}\x1b[0m",
                                    &kvc);
                               },
                                _ => {}
                                //End of match kvc.
...
///////////////////////////////////////////////////////////////////////////
	...
	//Create the tail of the command which pipes the data into 'action2' or 'action5'.  
		let resend1 = String::from("\", 1]}' |json_pp > ~/zector_detector_pub_zcashd/");
	...
	//Zzzzzzzz
		...
		//and the filename for the recieved curl data to be piped to.
		gtrwtxin.push_str(&sp4);
		...
		//This creates an inotify instance add_watch on the filename that receives the curl response.
			let mut inotify = Inotify::init().expect("Error while initializing inotify instance");
			inotify.add_watch(&sp4, WatchMask::CLOSE_WRITE).expect("Failed to add file watch");
	...
	//Either skip or print the data.
                            match kvc {
                                ...
                                "\"activationheight\"" | "\"confirmations\"" | "\"hash\"" | "\"nextblockhash\"" | "\"previousblockhash\"" => {
									println!(
                                    "\x1b[48;5;232m\x1b[38;5;228m{}{}{}\x1b[0m",
                                    &kvc,
                                    &klines.next().unwrap(), 
                                ...
                                "\"name\"" | "\"height\"" | "\"version\"" | "\"expiryheiht\"" | "\"anchor\"" | "\"overwintered\"" => {
									println!(
                                ...
                                "\"valueBalance\"" | "\"value\"" | "\"blockhash\"" | "\"vpub_old\"" | "\"vpub_new\"" => {
									println!(
                               ...
                               "\"coinbase\"" => {
                               println!(
                                    "\x1b[48;5;232m\x1b[38;5;210m**{}**\x1b[0m",
                                    &kvc);
                               },
	...
	//Pipe all the output of the cmd below to collection.txt
       ...
       let _now = time::Instant::now();
		...
		//This creates an inotify instance add_watch on the filename that receives the curl response.
			let mut inotify = Inotify::init().expect("Error while initializing inotify instance");
			inotify.add_watch(&sb4, WatchMask::CLOSE_WRITE).expect("Failed to add file watch");
	...
	//Either skip or print the data.
                            match kvc {
                                ...
                                erval += 1;
                                 ...
                                 "\"difficulty\"" | "\"merkleroot\"" | "\"nonce\"" | "\"size\"" | "\"version\"" | "\"blocks\"" => {
									println!(
                                    "\x1b[48;5;232m\x1b[38;5;10m{}{}{}\x1b[0m",
                                    &kvc,
                                    &klines.next().unwrap(), 
				    ...
				    println!();
                                ...
                                "\"chainwork\"" | "\"bits\"" | "\"anchor\"" | "\"name\"" | "\"height\"" => {
									println!(
